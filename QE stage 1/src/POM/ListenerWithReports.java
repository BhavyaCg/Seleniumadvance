...
import org.testng.annotations.Test;
	...
	//ITestListener is an interface unimplemented methods
	//like triggers in plsql
	// after adding implements ITestListener to class and importing org.testng.ITestListener; 
	//the classname will show red....mouse hover and add unimplemented methods
	// used in ListenerDemo.xml
	//used in VerifyLogin_pgfactory
	...
	FileUtils.copyFile(srcFile, new File("src/POM/test_" + result.getClass().getName()
	                    + AddTime()+"_screenshot.png"));
	...
	When running TestNG tests, one could want to perform some common actions
	 – after each test has finished successfully, after each failed test, 
	 after each skipped test, or  after all the tests have finished running, 
	 no matter their result. To apply such a common behavior to a group of tests, 
	 a custom listener can be created, that implements TestNG’s ITestListener interface.
